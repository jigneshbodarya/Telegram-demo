    ...
    return ^MTDatacenterAddressListData *(NSData *data) {
        ...
        if ([response isKindOfClass:[Api65_Config class]])
        {
            ...
            for (Api65_DcOption *dcOption in ((Api65_Config *)response).dcOptions)
            {
                ...
                {
                    MTDatacenterAddress *address = [[MTDatacenterAddress alloc]initWithIp:dcOption.ipAddress port:(uint16_t)[dcOption.port intValue]  preferForMedia:[dcOption.flags intValue] restrictToTcp:[dcOption.flags intValue] & (1 << 2) cdn:true];
