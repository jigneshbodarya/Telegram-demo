        ...
        else if ([update isKindOfClass:[TLUpdate$updateDcOptions class]])
        {
            ...
            for (TLDcOption$modernDcOption *datacenterOption in datacenterOptionsUpdate.dc_options)
            {
                if (datacenterOption.ip_address.length == 0)
                    continue;
//                [MTDatacenterAddress alloc] initWithIp:datacenterOption.ip_address port:(uint16_t)(datacenterOption.port == 0 ? 443 : datacenterOption.port) preferForMedia:false restrictToTcp:<#(bool)#> cdn:<#(bool)#>
               
                
                
                [[TGTelegramNetworking instance] mergeDatacenterAddress:datacenterOption.n_id address:[[MTDatacenterAddress alloc] initWithIp:datacenterOption.ip_address port:(uint16_t)(datacenterOption.port == 0 ? 443 : datacenterOption.port) preferForMedia:false restrictToTcp:datacenterOption.flags & (1 << 2) cdn: false]];
            }
        }
