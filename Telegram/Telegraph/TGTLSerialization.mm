    ...
    return ^MTDatacenterAddressListData *(NSData *response)
    {
        ...
        if ([result isKindOfClass:[TLConfig class]])
        {
            ...
            for (TLDcOption$modernDcOption *dcOption in ((TLConfig *)result).dc_options)
            {
                ...
                {
                    MTDatacenterAddress *address = [[MTDatacenterAddress alloc] initWithIp:dcOption.ip_address port:(uint16_t)dcOption.port preferForMedia:dcOption.flags & (1 << 1) restrictToTcp:dcOption.flags & (1 << 2) cdn: false];
